...
for doc in os.listdir(doc_path):
    ...
    if doc.endswith('txt'):
        ...
        # document = document.encode(encoding='utf-8', errors='replace')

        # Get List of candidates
        candidates = extract_candidate_keywords(document)

        print('<Bug> <id> 17 </Id> <Name> CategoryKeyNotFoundException </Name> <CreatedDate> 27-03-2022 </CreatedDate> '
              '</Bug> <Solution> <Id> 11 </Id> <Solution> <Please do IIS reset since new configurations are added to '
              'the system> <CreatedDate> 27-03-2022 </CreatedDate> </Solution> ')
...
print('number of keywords extracted', len([x for x, y in feature_list if y == 0]))
